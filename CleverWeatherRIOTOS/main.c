...
static int cmd_publisher(int argc, char **argv){
    /*generate random values and publish them on the topic of your choice*/
    emcute_topic_t t;
    unsigned flags = EMCUTE_QOS_0;

    if (argc < 2){
        printf("usage: <topic name>\n");
        return 1;
    }
    ...
    while (1){
        ...
        /* step 2: publish data */
        if (emcute_pub(&t, mess, sizeof(mess), flags) != EMCUTE_OK){
            printf("error: unable to publish data to topic '%s [%i]'\n", t.name, (int)t.id);
            return 1;
        }

        printf("Published %i bytes to topic '%s [%i]'\n", sizeof(mess), t.name, t.id);
...
static const shell_command_t shell_commands[] = {
    ...
    {"publisher", "start publishing messages on a topic of your choice", cmd_publisher},
    { NULL, NULL, NULL }
};
